...
function getStyle(iso, userPath, targetCountry, hintCountry, hintLevel) {
  ...
  if (isHint)   return { fill: '#fcd34d', stroke: '#d97706', strokeWidth: 0.5, opacity: 1 } 
  ...
  if (hintLevel === 1) return { fill: '#f1f5f9', stroke: '#cbd5e1', strokeWidth: 0.2, opacity: 0.8 } 
...
const WorldMap = memo(function WorldMap({
  userPath, targetCountry, hintCountry, hintLevel,
}) {
  const [position, setPosition] = useState({ coordinates: [0, 15], zoom: 1 })

  // محاسبه خودکار دوربین نقشه (کادربندی هوشمند)
  useEffect(() => {
    ...
    if (validCount > 0) {
      ...
      // اصلاح محاسبه برای زمانی که مبدا و مقصد از اقیانوس آرام رد میشن (مثل آمریکا و ژاپن)
      if (lonDiff > 180) {
        lonDiff = 360 - lonDiff
      ...
      // پیدا کردن بیشترین فاصله برای محاسبه میزان زوم
      const maxDiff = Math.max(lonDiff, latDiff, 25) 
      
      // فرمول محاسبه زوم داینامیک (0.45 ضریبی است که باعث میشه کشورها قشنگ کل کادر رو پر کنن)
  ...
  return (
    <div className="w-full h-full relative overflow-hidden bg-transparent z-0 flex flex-col items-center justify-center">
      
      {(hintLevel === 1 || hintLevel === 2) && (
        <div className="absolute top-2 inset-x-0 z-10 mx-auto w-max px-3 py-1 bg-white/90 backdrop-blur-sm rounded-full shadow-sm text-[10px] sm:text-xs font-semibold">
          {hintLevel === 1 ? (
            <span className="text-amber-600">کشور زرد کجاست؟ (گام بعدی)</span>
          ) : (
            <span className="text-slate-500">نقشه جهان نمایش داده شد</span>
          )}
        </div>
      )}

      {/* بوم نقشه که قابلیت زوم شدن داره */}
      <div className="w-full h-full flex-1 flex items-center justify-center pointer-events-none">
        <ComposableMap
          projectionConfig={{ scale: 140 }}
          style={{ width: '100%', height: '100%', objectFit: 'contain' }}
        >
          {/* گروه زوم که دوربین رو کنترل می‌کنه */}
          <ZoomableGroup
            center={position.coordinates}
            zoom={position.zoom}
            transitionDuration={800} // افکت نرم هنگام جابجایی دوربین
            minZoom={1}
            maxZoom={6}
          >
              ...
              {({ geographies }) =>
                geographies.map(geo => {
                  ...
                  const style = {
                    fill: s.fill,
                    stroke: s.stroke,
                    strokeWidth: s.strokeWidth / position.zoom, // خط دور کشورها با زوم کردن ضخیم نمیشه
                    opacity: s.opacity,
                    outline: 'none',
      ...
      <div className="absolute bottom-2 inset-x-0 flex items-center justify-center gap-4 flex-wrap z-10 pointer-events-none">
        <span className="flex items-center gap-1.5 text-[10px] text-slate-500 bg-white/80 px-2 py-0.5 rounded-full backdrop-blur-sm shadow-sm">
          <span className="w-2 h-2 rounded-full bg-[#4ade80]" /> مبدا
        </span>
        <span className="flex items-center gap-1.5 text-[10px] text-slate-500 bg-white/80 px-2 py-0.5 rounded-full backdrop-blur-sm shadow-sm">
          <span className="w-2 h-2 rounded-full bg-[#f87171]" /> مقصد
        </span>
        {userPath.length > 1 && (
          <span className="flex items-center gap-1.5 text-[10px] text-slate-500 bg-white/80 px-2 py-0.5 rounded-full backdrop-blur-sm shadow-sm">
            <span className="w-2 h-2 rounded-full bg-[#60a5fa]" /> مسیر شما
          </span>
        )}
      </div>
    </div>
